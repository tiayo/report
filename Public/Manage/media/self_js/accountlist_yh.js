...
Account = {
	...
	report : function(){
		$(document).on('click','#report',function(){
			...
			Account.params.sstype = $('[name=sstype] option:selected').val();
			Account.params.status = $('[name=status] option:selected').val();
			Account.params.report = 1;

			window.location.href = reportUrl + "?" + jQuery.param(Account.params);
		});
	},
		...
		if(list.length>0){
			$.each(list,function(k,v){
				html += '<tr>'+
				  			...
				  			'<td class="bankicon"><span class="badge" style="cursor:pointer;" url="'+rooturl+v.cardpositive+'" url2="'+rooturl+v.cardnegative+'">'+v.idcardnumber+'</span></td>'+
				  			// '<td class="bankicon">'+
	...
	bankicon : function(){
		$('.bankicon span').unbind('click').bind('click',function(event){
			...
			layer.open({
			    ...
			    area: ['auto','50%'],
			    shadeClose: true, //开启遮罩关闭
			    content: '<img style="width:50%;height:50%" src="'+url+'"> <img style="width:50%;height:50%" src="'+url2+'">'
			});
		});
	},
